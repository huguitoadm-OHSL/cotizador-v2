...
export default function App() {
          ...
          const normalizedData = dataArray.map(item => {
            ...
            const cleanNumber = (val) => {
                ...
                if (strVal.includes(',') && strVal.includes('.')) {
                    strVal = strVal.indexOf(',') > strVal.indexOf('.') ? strVal.replace(/\./g, '').replace(',', '.') : strVal.replace(/,/g, ''); 
  ...
  const handleReset = () => {
    setResultado(null); setEscenarioA(null); setShowComparativa(false); setShowTablaPagos(false);
    setUv(""); setMzn(""); setLote(""); setSuperficie(""); setPrecio(""); setCategoriaLote("");
    setAños(""); setInicialMonto(""); setInicialPorcentaje(""); setNombreCliente(""); window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  // FUNCIÓN MAGICA PARA EL PDF
  const generarPDF = () => {
    setShowTablaPagos(true); // Forzamos mostrar la tabla para que salga en el PDF
    setTimeout(() => {
      window.print(); // Llama a la función de impresión nativa del sistema
    }, 300);
  };
  ...
  return (
    <div className="min-h-screen bg-[#0B1121] relative font-['Plus_Jakarta_Sans'] text-slate-300 overflow-x-hidden selection:bg-cyan-500/30 selection:text-cyan-100">
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        @media print {
          /* Fondo blanco para no gastar tinta */
          ...
          * { text-shadow: none !important; }
          ...
          .text-cyan-400 { color: #0891b2 !important; } /* Cyan más oscuro */
          .text-emerald-400 { color: #059669 !important; } /* Emerald más oscuro */
            ...
            {!resultado ? (
              ...
              <div className="glass-panel-right rounded-[2.5rem] p-7 sm:p-10 relative overflow-hidden">
                ...
                <div className="hidden print:block text-center mb-8 border-b border-slate-300 pb-6">
                  {/* Aquí en local mostrará un texto alternativo si no encuentra tu logo, pero en tu Vercel cargará tu logo cuadradito */}
